...
function calcAbv(bst) {
  // Maps BST 200-680 to 3.5-11.5% ABV
  const pct = Math.min(1, Math.max(0, (bst - 200) / 480));
  return (3.5 + pct * 8.0).toFixed(1);
}

function abvDots(bst) {
  const abv = parseFloat(calcAbv(bst));
  // 5 dots: each dot = ~1.6% ABV above baseline of 3.5
  const filled = Math.round(Math.min(5, Math.max(1, (abv - 3.5) / 1.6)));
  let html = '<div class="abv-dots">';
  for (let i = 0; i < 5; i++) {
    html += `<div class="abv-dot ${i < filled ? 'filled' : 'empty'}"></div>`;
  }
  html += '</div>';
  return html;
}
...
function renderPokemonCard(pokemon, onClick, selected, dexCaught = false) {
  const primaryType = (pokemon.types || ['Blonde'])[0];
  const typeClass = 'tc-' + primaryType.toLowerCase().replace('barrel-aged','barrelaged').replace(/-/g,'');
  const bst = pokemon.baseStats ? Object.values(pokemon.baseStats).reduce((a,b)=>a+b,0) : 400;
  const abv = calcAbv(bst);
  const batchNum = String(pokemon.level).padStart(3,'0');
  const displayName = pokemon.nickname || pokemon.brewName || pokemon.name;
  const [brewName, brewStyle] = (() => {
    const full = displayName;
    const styleWords = ['Session','Imperial','Double','Farmhouse','Amber','Märzen'];
  ...
  const heldHtml = (pokemon.heldItems||[]).length > 0
    ? `<div class="label-items">${(pokemon.heldItems||[]).map(it=>`<span class="label-item-pill" title="${it.desc}">${itemIconHtml(it,12)} ${it.name}</span>`).join('')}</div>`
    : '';
  const shinyBorder = pokemon.isShiny ? ' style="border-color:gold;"' : '';
  const dexBadge = dexCaught ? '<img class="dex-caught-badge" src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/items/poke-ball.png" alt="Caught" title="Already in Brewlog">' : '';
  return `<div class="poke-card ${typeClass}${selected?' selected':''}" ${onClick?'role="button" tabindex="0"':''}>
    <div class="label-stripe"></div>
    <div class="label-inner"${shinyBorder}>
      <div class="label-brewery">${(typeof state !== "undefined" && state.breweryName) || "Nonsense Sloth Co."}</div>
      <div class="label-divider"></div>
      <div class="label-sprite-wrap">
        <img src="${pokemon.spriteUrl || ''}" alt="${displayName}"
             class="poke-sprite" onerror="this.src='';this.style.display='none'">
        ${pokemon.isShiny ? '<span class="label-shiny">✦</span>' : ''}
        ${dexBadge}
      </div>
      <div class="label-name">${brewName}</div>
      <div class="label-style">${brewStyle}</div>
      <div class="label-divider-sm"></div>
      <div class="label-types">
        <span class="label-type-pill ${typeClass}">${primaryType}</span>
        ${secondType ? `<span class="label-type-pill ${secondTypeClass}">${secondType}</span>` : ''}
      </div>
      <div class="label-hp-row">
        <span class="label-stat-key">HP</span>
        <div class="label-hp-track"><div class="label-hp-fill" style="width:${Math.max(0,Math.floor(pokemon.currentHp/pokemon.maxHp*100))}%"></div></div>
        <span class="label-hp-val">${Math.max(0,pokemon.currentHp)}/${pokemon.maxHp}</span>
      </div>
      ...
      <div class="label-footer">
        <div class="label-abv">
          <span class="label-abv-val">${abv}%</span>
          <span class="label-abv-label">ABV</span>
        </div>
        <div class="label-batch">
          <span class="label-batch-val">${batchNum}</span>
          <span class="label-batch-label">Batch</span>
        </div>
      </div>
    </div>
  </div>`;
...
function getMoveForPokemon(pokemon) {
  return getBestMove(pokemon.types || ['Blonde'], pokemon.baseStats, pokemon.speciesId, pokemon.moveTier ?? 1);
}
...
function renderTeamBar(team, el) {
  ...
  team.forEach((p, i) => {
    ...
    slot.innerHTML = `
      ...
      <div class="team-slot-items">${(p.heldItems||[]).map((it,si) =>
        `<div class="team-slot-item" data-item-slot="${si}" title="${it.name}: ${it.desc}">${itemIconHtml(it, 16)}</div>`
      ).join('')}</div>`;
    ...
    if (isMain && (p.heldItems||[]).length > 0) {
      slot.querySelectorAll('.team-slot-item').forEach(itemEl => {
        ...
        itemEl.addEventListener('click', e => {
          e.stopPropagation();
          hideTeamHoverCard();
          openItemEquipModal((p.heldItems||[])[slotIdx], {
            fromPokemonIdx: i,
            fromPokemonItemIdx: slotIdx,
...
const TYPE_COLORS_RGB = {
  blonde:'200,200,200', Red:'255,120,30', lager:'60,140,255',
  sour:'255,220,0', ipa:'50,200,50', cryo:'150,220,255',
  barleywine:'220,60,30', brett:'160,60,220', brown:'180,140,60',
  wheat:'130,180,255', belgian:'255,80,180', saison:'100,200,50',
  "barrel-aged":'160,130,80', seltzer:'100,60,180', stout:'60,80,220',
};
...
function animRedPunch(canvas, ctx, from, to) {
...
function animLagerfall(canvas, ctx, from, to) {
  return runCanvas(canvas, ctx, 500, (ctx, t) => {
    if (t < 0.5) {
      // Lager column falling from above onto target
      const st = t / 0.5;
...
function animRazorLeaf(canvas, ctx, from, to) {
  // 3 leaves wheat to target in spread
...
function animCryoPunch(canvas, ctx, from, to) {
  return runCanvas(canvas, ctx, 380, (ctx, t) => {
    if (t < 0.35) {
      const st = t/0.35;
      const ex=lerp(from.x,to.x,st), ey=lerp(from.y,to.y,st);
...
function animBrettJab(canvas, ctx, from, to) {
  return runCanvas(canvas, ctx, 380, (ctx, t) => {
    if (t < 0.4) {
      const st=t/0.4;
      const ex=lerp(from.x,to.x,st), ey=lerp(from.y,to.y,st);
...
function animEarthquake(canvas, ctx, from, to) {
  // Brown shockwave rings spreading from attacker through target
...
function animStoutClaw(canvas, ctx, from, to) {
  return runCanvas(canvas, ctx, 400, (ctx, t) => {
    if(t<0.4) {
      const st=t/0.4;
      const ex=lerp(from.x,to.x,st), ey=lerp(from.y,to.y,st);
...
function animSolarBeam(canvas, ctx, from, to) {
  // Phase 1: charge up (golden orb at attacker) | Phase 2: beam Reds
...
function animSaisonBuzz(canvas, ctx, from, to) {
  const dx=to.x-from.x, dy=to.y-from.y;
  return runCanvas(canvas, ctx, 600, (ctx, t) => {
...
function animStoutPulse(canvas, ctx, from, to) {
  const dx=to.x-from.x, dy=to.y-from.y;
  const dist=Math.hypot(dx,dy);
  const ang=Math.atan2(dy,dx);
  return runCanvas(canvas, ctx, 600, (ctx, t) => {
    // Stout-shaped energy wave
    const progress=t;
    const cx=from.x+dx*progress, cy=from.y+dy*progress;
    // Main wave
...
function animSplash(canvas, ctx, from, to) {
  // Lager droplets arc up from the attacker and fall back down
  return runCanvas(canvas, ctx, 700, (ctx, t) => {
    const drops = [
...
function animTeleport(canvas, ctx, from, to) {
  // Expanding belgian rings burst from the attacker, then a quick flash
  return runCanvas(canvas, ctx, 500, (ctx, t) => {
...
function playAttackAnimation(moveType, attackerEl, targetEl, isSpecial = true, moveName = '') {
  ...
  if (!isSpecial) {
    ...
    switch(moveName) {
      ...
      case 'Red Punch':   return animRedPunch(canvas, ctx, from, to);
      case 'Lagerfall':    return animLagerfall(canvas, ctx, from, to);
      case 'Thunder Punch':return animThunderPunch(canvas, ctx, from, to);
      case 'Razor Leaf':   return runParticleCanvas(canvas, ctx, buildParticles('ipa', from, to), 650);
      case 'Cryo Punch':    return animCryoPunch(canvas, ctx, from, to);
      case 'Close Combat': return animCloseConbat(canvas, ctx, from, to);
      case 'Brett Jab':   return animBrettJab(canvas, ctx, from, to);
      ...
      case 'Stout Claw':  return animStoutClaw(canvas, ctx, from, to);
      default: {
    ...
    switch(moveName) {
      ...
      // Surf, Thunderbolt use the existing buildParticles animations (lager/sour are great)
      ...
      case 'Saison Buzz':     return animSaisonBuzz(canvas, ctx, from, to);
      case 'Power Gem':    return animPowerGem(canvas, ctx, from, to);
      case 'Shadow Ball':  return animShadowBall(canvas, ctx, from, to);
      case 'Stout Pulse': return animStoutPulse(canvas, ctx, from, to);
      default: {
        // Use existing buildParticles for remaining special moves (Flamethrower, Surf, Thunderbolt, Cryo Beam, Belgian)
        const type = (moveType || 'blonde').toLowerCase();
        const particles = buildParticles(type, from, to);
        const duration = type === 'sour' ? 550 : type === 'belgian' ? 700 : 650;
...
function buildParticles(type, from, to) {
  ...
  if (type === 'Red') {
    // Flamethrower: sustained jet of Red streaming from attacker to target.
    ...
    // Lager Gun: a coherent pressurised stream (grows like the cryo beam but wavy+blue)
    let streamAge = 0;
    ps.push({
      alive: true,
      ...
      draw(ctx) {
        ...
        const phase = streamAge * 0.012; // scrolling phase = lager flowing
        ctx.beginPath();
    ...
    // Cryo Beam: expanding cyan beam + crystal shards at impact
    let beamAge = 0;
    ps.push({
      alive: true,
    ...
    // Belgian: pink expanding rings + orbiting sparkles
    let pAge = 0;
    ...
    // Saison Buzz: yellow-green spore cloud
    for (let i = 0; i < 25; i++) {
      const delay = i * 15;
    ...
    // Stout Rage: rainbow energy beam
    let dAge = 0;
...
function openPokedexModal(initialTab = 'normal') {
  ...
  function buildNormalGrid() {
    ...
    const grid = [...ALL_CATCHABLE_IDS].sort((a,b) => a-b).map(id => {
      const e = dex[id];
      if (e) {
  ...
  modal.innerHTML = `
    <div class="dex-modal-box">
      <div class="dex-modal-header">
        <div class="dex-tabs">
          <button class="dex-tab" data-tab="normal">📖 Brewlog</button>
...
const PATCH_NOTES = [
  {
    ...
    sections: [
      ...
      },
      {
        ...
        entries: [
          'The ? node now triggers real brewery random events instead of secretly being another node type',
          '🍺 Surprise Tap Release — pick one of 3 items from a special batch',
          '👨‍🍳 Head Brewer Returns — whole team fully restored',
          '🏅 Medal Won — whole team gains a level',
          '🧫 Perfect Yeast Pitch — pick one brew to fully restore',
          '⚗️ Explore a New Technique — 50/50: chosen brew gains 2 levels or takes 25% damage',
          '🥣 Stuck Sparge — cut your losses (lose an item) or push through (team takes 15% damage)',
          '🔄 Tap Takeover Night — optionally swap a brew for a same-quality visiting tap',
          '🌡️ Glycol Chiller Failed — whole team loses 10% HP',
          '📋 OSHA Inspection — inspectors confiscate one item',
          '⬇️ Keg Recall — your strongest brew drops a level',
          'Bad events have lower weight than good/risky ones so they feel fair',
        ],
      },
      {
        heading: '🌾 Item Overhaul',
        entries: [
          'All items renamed to brewing terms — Life Orb → Banquet Beer Stash, Choice Band → European Malt, etc.',
          'Physical damage items now described as flavor boosts; special damage as aroma boosts',
          'All 18 beer types now have a matching type-boost item: Hop Kief (Cryo), Dehusked Malts (Cascadian), Marshmallow Fluff (Pastry), Trade Agreement (Export)',
          'Each pokemon can now hold up to 3 items simultaneously',
          'Items are returned to your bag when a pokemon is released or swapped out',
        ],
      },
      {
        heading: '🗺️ Map & Naming',
        entries: [
          'Pokemon Center renamed to QC Lab',
          'Catch node renamed to Brew New Batch',
          'Pokédex renamed to Brewlog',
          'Map legend updated to reflect all renamed nodes',
          'QC Lab guaranteed on layer 6 of every map',
          'Extra map layer added — layer 5 is now a resource-only pit stop (catch/item/event, no battles)',
        ],
      },
      {
        heading: '⚔️ Battle Balance',
        entries: [
          'Type effectiveness capped per interaction: single super effective 2x → 1.5x, double 4x → 2.25x',
          'STAB remains 1.5x and is applied independently from type effectiveness',
          'Expert Belt now triggers at ≥1.5x effectiveness to match the new cap',
          'Move power tuned: Lager physical 82, Brett physical 82, Red special 94, Wheat physical 81 / special 93, Blonde physical lowered to 80 so Wheat/Blonde pokemon correctly use Wheat moves',
          'Dual-type pokemon now use their strongest available move across both types',
        ],
      },
      {
        heading: '📖 Expanded Roster',
        entries: [
          'Catchable pokemon expanded from Gen 1 (151) to Gen 1–3 (386)',
          'All pokemon stats now loaded from a local species.js file — faster and works offline',
          'Evolution chains updated to include all Gen 2 and Gen 3 evolutions',
          'Eevee can now evolve into Espeon (Belgian) and Umbreon (Cascadian)',
        ],
      },
      {
        heading: '🏷️ Brew Names & Nicknames',
        entries: [
          'Every brew now has a unique brewery-style name — Gengar becomes "Gen Gar Imperial Seltzer", Gyarados becomes "Guy A Dos Imperial Lager Wheat"',
          'Brew names shown everywhere in place of the original species name',
          'Nickname any brew when you catch it or at any QC Lab — your nickname takes priority',
          'Nicknames saved in the Hall of Fame and Brewlog',
          'Cards now show ABV% derived from BST (stronger pokemon = higher ABV) and a Batch number matching the brew\'s level',
        ],
      },
    ],
  },
    ...
    sections: [
      ...
      },
      {
        ...
        entries: [
          ...
          'The first Catch node on Map 1 always includes a IPA or Lager type Pokémon',
        ],
      },
    ],
  },
...
function openPatchNotesModal() {
  ...
  modal.innerHTML = `
    <div style="background:var(--bg-main);border:2px solid var(--border);border-radius:12px;width:90%;max-width:500px;max-height:80vh;display:flex;flex-direction:column;font-family:'Press Start 2P',monospace;">
      ...
      <div style="padding:10px 16px 0;border-bottom:1px solid var(--border);font-family:'Press Start 2P',monospace;">
        <p style="font-size:8px;color:var(--text-dim);line-height:1.8;margin:0 0 10px;">
          Inspired by <a href="https://pokelike.xyz" target="_blank" style="color:#4af;text-decoration:none;">pokelike.xyz</a> — the project that started it all. 🍺
        </p>
      </div>
...
// ---- Brewery Name Randomozer ----
const adjectives = [
  "Nonsensical", "Critical", "Hoppy", "Drunken", "Wild", "Rustic", 
  "Iron", "Standard", "Foggy", "Bitter", "High-Volume", "Barrel-Aged",
  ...
  "Berserker", "Wandering", "Ominous", "Misty", "Forgotten",
  "Tainted", "Astral", "Ancient", "Prismatic", "Grim",
 "Pitch-Black", "Deep-Holed", "Hard-Logged", "Thirsty", "Heavy-Handed",
  "Dirty", "Sticky", "Slippery", "Double-Fisted", "Back-Side",
  "Wasted", "Tipsy", "Unfiltered", "Naked", "Saturating",
...
const nouns = [
  "Sloth", "Mash", "Cellars", "Fermentarium", "Hop Works", 
  "Batch", "Kettle", "Lager Co", "Vault", "Ales",
...
const nameInput = document.getElementById('brewery-name-input');const randomBtn = document.getElementById('brewery-randomize');
...
randomBtn.addEventListener('click', () => {
    ...
    // Ensure it doesn't exceed 28 char limit
    nameInput.value = fullName.substring(0, 28);
});

// ---- Run Modifiers Modal ----

function openModifiersModal() {
  const existing = document.getElementById('modifiers-modal');
  if (existing) { existing.remove(); return; }

  const active = getActiveModifiers();

  // Build conflict map: id -> Set of conflicting ids
  const conflictMap = {};
  for (const m of RUN_MODIFIERS) {
    conflictMap[m.id] = new Set(m.conflicts);
    for (const c of m.conflicts) {
      if (!conflictMap[c]) conflictMap[c] = new Set();
      conflictMap[c].add(m.id);
    }
  }
  ...
  function render() {
    const activeNow = getActiveModifiers();

    // Compute which modifiers conflict with the current active set
    const warnSet = new Set();
    for (const id of activeNow) {
      for (const conflict of (conflictMap[id] || [])) {
        if (activeNow.has(conflict)) warnSet.add(id), warnSet.add(conflict);
      }
    }
    ...
    modal.innerHTML = `
      <div class="mod-modal-box">
        ...
        ${activeNow.size > 0 ? `
          <div class="mod-active-summary">
            Active: ${[...activeNow].map(id => {
              const m = RUN_MODIFIERS.find(x => x.id === id);
              return m ? `${m.icon} ${m.name}` : id;
            }).join(' · ')}
          </div>` : ''}
        <div style="text-align:center;margin-top:14px;">
          <button class="btn-secondary" style="font-size:9px;" onclick="clearModifiers()">Clear All</button>
        </div>
      </div>`;
